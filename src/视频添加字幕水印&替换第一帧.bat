...
rem 16*2=32 9*2=18
set prefix=ffmpeg -i "%video%" -i "%cover%" -i logo.png -filter_complex "[0:v]ass=%subtitle%[subtitled];[subtitled][2:v]overlay=W-w-32:18[watermarked];[1:v]scale=1920:1080[cover];[watermarked][cover]overlay=x='if(eq(t,0),0,NaN)':y=0" -c:v h264 -profile:v main -level 4.2 
